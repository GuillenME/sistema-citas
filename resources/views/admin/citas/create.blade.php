...
@section('styles')
    ...
    <link rel="stylesheet" href="{{ asset('css/admin/citas-create.css') }}">
...
@section('content')
    <div class="admin-citas-create">
    <div class="container">
        <div class="card booking-shell">

            <div class="booking-head">
                <h2>Agendar Cita</h2>
                <p>Registra una cita como administrador. Selecciona cliente, servicio, fecha y hora disponible.</p>
            </div>
            ...
            <form method="POST" action="{{ route('admin.citas.store') }}" id="formAgendarCita" class="form-grid booking-grid">
                @csrf
                ...
                <div class="booking-left">
                    ...
                    <div class="field date-field">
                        ...
                        <input type="text" id="fecha" name="fecha" class="date-inline" placeholder="Selecciona una fecha"
                            value="{{ old('fecha') }}" onkeydown="return false;" readonly required>
                    </div>
                ...
                <div class="field anticipo-field booking-summary">
                    <h4>Resumen de Cita <span class="summary-pill">Admin</span></h4>
                    ...
                    <div class="field">
                        <label class="privacy-label">
                            <input type="checkbox" id="anticipo_check" name="anticipo_recibido" value="1" class="privacy-checkbox" required {{ old('anticipo_recibido') ? 'checked' : '' }}>
                            <span class="privacy-text">Se recibio anticipo</span>
                        </label>
                    </div>
                    <div class="field" id="anticipo_box" hidden>
                        ...
                        <input type="number" id="anticipo_monto" name="anticipo_monto" min="0.01" step="0.01"
                            placeholder="Ej. 100.00" value="{{ old('anticipo_monto') }}">
                    </div>
    ...
    <div id="modalServicioConfirmar" class="modal-confirm-overlay" onclick="if(event.target === this) cancelarServicio()">
        <div class="modal-confirm-content modal-servicio">
            ...
            <p class="modal-servicio-info"><strong>Duracion:</strong> <span id="msDuracion"></span> minutos</p>
            <p class="modal-servicio-info"><strong>Precio:</strong> <span id="msPrecio"></span></p>
            <div class="modal-confirm-buttons">
                <button class="modal-confirm-btn modal-confirm-btn-submit" onclick="confirmarServicio()">Confirmar servicio</button>
                <button class="modal-confirm-btn modal-confirm-btn-cancel" onclick="cancelarServicio()">Cambiar servicio</button>
            </div>
        </div>
    </div>
    ...
    <div id="modalConfirmar" class="modal-confirm-overlay" onclick="if(event.target === this) cerrarModalConfirmar()">
        <div class="modal-confirm-content">
            ...
            <p class="modal-warning-note">Si registras anticipo, la cita se guardara como confirmada.</p>
            <div class="modal-confirm-buttons modal-confirm-buttons-spaced">
                <button class="modal-confirm-btn modal-confirm-btn-submit" onclick="confirmarAgendar()">Confirmar y agendar</button>
                <button class="modal-confirm-btn modal-confirm-btn-cancel" onclick="cerrarModalConfirmar()">Cancelar</button>
            </div>
        </div>
    </div>
...
@section('scripts')
    <script>
        document.body.dataset.anticipoPct = "{{ (float) config('citas.porcentaje_anticipo', 50) }}";
    </script>
